  ...
  return (
    ...
    {newsItems.map((item, index) => (
      <div key={index} className='flex flex-col '>
        ...
        <h1 className='text-[28px] font-medium mt-4'>{item.title}</h1>
        <h1 className='text-[28px] font-medium'>{item.date}</h1>
        <button className='bg-green w-fit py-3 px-6 flex items-center gap-2 rounded-xl text-white mt-6'>{item.buttonText} <FiArrowRight /></button>
      </div>
    ))}
  </div>
        </div>
